...
export const sessionHandler = functions.https.onRequest(
  async (request: any, response: functions.Response<FunctionsResponse>) => {
    ...
    try {
      // Identify stitches together anonymous sessions
      if (body?.type === 'identify') {
        // Sync anonymousId to userId
        if (
          body.userId &&
          body.anonymousId &&
          body.anonymousId !== 'userNotTracked'
        ) {
          // Get the current user's session and sync it to the current session
          const anonymousSessionsDoc = await firebaseAdmin
            .firestore()
            .collection(FirestoreCollection.SESSIONS)
            .doc(body.anonymousId)
            .get();

          const anonymousSessions = ((await anonymousSessionsDoc.data()) ??
            []) as IUserSession[];

          dlog('sessions ➡️ anonymousSessions:', anonymousSessions);
          ...
          //   const mergedSessions = mergeSessions(anonymousSessions, userSessions);
          ...
          await firebaseAdmin
            ...
            .doc(body.userId)
            .set({
              id: firebaseAdmin.firestore.FieldValue.arrayUnion(
                anonymousSessions,
              ),
            });

          // Remove original anonymous session
          firebaseAdmin
            .firestore()
            .collection(FirestoreCollection.SESSIONS)
            .doc(body.anonymousId)
